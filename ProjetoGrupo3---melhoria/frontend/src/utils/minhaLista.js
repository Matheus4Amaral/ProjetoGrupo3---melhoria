...
export async function adicionarNaLista(itemData) {
  ...
  const tipo = itemData.tipo || "movie";

  // Verifica se o item já está na lista antes de tentar adicionar de novo
  const jaEstaNaLista = await estaNaLista(itemData.id, tipo);
  if (jaEstaNaLista) {
    return { sucesso: false, mensagem: "Este item já está na sua lista." };
  }
